...
private static final AppLogger LOG = AppLoggerFactory.getLogger(HomeSearchServiceImpl.class, AppLoggingDomain.OCLUI);
    ...
    private Email prepareDailyEmailData() {
        Email email = new Email();
        String recipientsFromDb = homeSearchDao.getDailyMailRecipients();
        if (recipientsFromDb != null && recipientsFromDb.trim().length() > 0) {
...
private static final AppLogger LOG = AppLoggerFactory.getLogger(HomeSearchServiceImpl.class, AppLoggingDomain.OCLUI);
    ...
    private static final int HOMESEARCH_EMAIL_ERR_CD = 18000;
    ...
    // Existing methods (searchErrored, exportExcelDocument, checkDocumentListType, etc.) assumed here

    /**
     * US-18451: Prepare daily search criteria for errored documents report.
     * @return HomeSearchDetails with documentType "UNORTH" and last 24h date range.
     */
    // US-18451
    private HomeSearchDetails prepareHomeSearchDetailsForDailyJob() {
        HomeSearchDetails details = new HomeSearchDetails();
    ...
    /**
     * US-18451: Add attachment data to Email bean.
    ...
    /**
     * US-18451: Prepare Email object with recipients, from, subject.
...
private static final com.hpe.ocl.adminUI.common.logging.AppLogger LOG = com.hpe.ocl.adminUI.common.logging.AppLoggerFactory.getLogger(HomeSearchServiceImpl.class, com.hpe.ocl.adminUI.common.logging.AppLoggingDomain.OCLUI);
    ...
    private static final String UNORTH_DOCUMENT_TYPE = "UNORTH";
    private static final String EMAIL_ATTACHMENT_FILENAME = "Daily_Errored_Documents.xlsx";
    private static final String EMAIL_ATTACHMENT_CONTENT_TYPE = "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet";
    private static final int HOMESEARCH_EMAIL_ERR_CD = 5001; // assuming error code constant

    /**
     * US-18451: Prepare HomeSearchDetails for the daily errored document report.
     * Sets document type UNORTH and date range to last 24 hours.
     * Creates a system UserProfile and calls existing searchErrored() method.
     *
     * @return HomeSearchDetails list of errored documents
     */
    // US-18451
    private HomeSearchDetails prepareHomeSearchDetailsForDailyJob() {
        HomeSearchDetails details = new HomeSearchDetails();
        details.setDocumentType(UNORTH_DOCUMENT_TYPE);
     ...
     *
     * @param email the Email object to add attachment to
     * @param fileName the name of the attachment file
     * @param fileContent byte array content of the attachment
    ...
    private Context prepareMailBodyForDailyJob() {
        Context ctx = new Context();
        if (homeSearchDetails != null) {
            ctx.setVariable("documentList", checkDocumentListType());
        }
        return ctx;
    }
    ...
    public void sendDailyDocumentListMail() {
        MDC.put("correlationId", "US-18451-DailyErroredDocReport");
        LOG.info("US-18451: Starting daily errored document report email job");
        try {
            this.homeSearchDetails = prepareHomeSearchDetailsForDailyJob();
            ...
            Email email = prepareDailyEmailData();
            addAttachment(email, EMAIL_ATTACHMENT_FILENAME, outputStream.toByteArray(), EMAIL_ATTACHMENT_CONTENT_TYPE);

            sendEmail.sendMailWithAttachment(email);
            LOG.info("US-18451: Daily errored document report email sent successfully");

        } catch (Exception e) {
            LOG.error("US-18451: Exception in sendDailyDocumentListMail", e);
            throw e;
        } finally {
    ...
    // US-18451
    @Override
    public java.util.List<?> checkDocumentListType() {
        java.util.List<?> docList = super.checkDocumentListType();
        if (docList == null) {
            return Collections.emptyList();
        }
        return docList;
